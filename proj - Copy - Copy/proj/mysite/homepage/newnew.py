...
def menu_view(request):
    ...
    if request.method == 'POST':
        ...
        i = i + str(1)
        while(flag):
            texttospeech("To access your your email say mail. To check maps say maps. To open Sent folder say Sent. To open Trash folder say Trash. To Logout say Logout. Do you want me to repeat?", file + i)
            i = i + str(1)
            say = speechtotext(3)
            if say == 'No' or say == 'no':
                flag = False
        ...
        act = act.lower()
        if act == 'mail':
            return JsonResponse({'result' : 'login'})
        elif act == 'map':
...
def map_view(request):
    global i, addr, passwrd , fname , lname, mail , ans 
    if request.method == 'POST':
        flag = True
        text1 = "Welcome to map section , where you can search for places and lookup for directions."
        texttospeech(text1, file + i)
        i = i + str(1)

        
        while (flag):
            texttospeech("to search places say search. to see direction say direction", file + i)
            i = i + str(1)
            ans= speechtotext(10)
            if ans != 'N':
                texttospeech("You meant " + ans+ " say yes to confirm or no to enter again", file + i)
                i = i + str(1)
                say = speechtotext(3)
                if say == 'No' or say == 'no':
                    flag = False
        ...
        act = act.lower()
        if act == 'search':
            return JsonResponse({'result' : 'search_view'})
        elif act == 'direction':
            return JsonResponse({'result' : 'direction_view'})           
        else:
            texttospeech("Invalid action. Please try again.", file + i)
            i = i + str(1)
            return JsonResponse({'result': 'failure'})
...
def search_view(request):
    ...
    if request.method == 'POST':
        text1 = "Welcome to our search section. what do you wanna search. "
        texttospeech(text1, file + i)
        i = i + str(1)    
        flag = True
        while (flag):
